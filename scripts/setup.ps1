...
if (-Not (Test-Path $PREMAKE_EXE)) {
    Write-Host "`n[1/11] Downloading Premake5 v$PREMAKE_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[1/11] Premake5 already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $QUILL_DIR "include\quill\Backend.h"))) {
    Write-Host "`n[2/11] Downloading Quill v$QUILL_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[2/11] Quill already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $YAMLCPP_DIR "include\yaml-cpp\yaml.h"))) {
    Write-Host "`n[3/11] Downloading yaml-cpp v$YAMLCPP_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[3/11] yaml-cpp already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $GTEST_DIR "googletest\include\gtest\gtest.h"))) {
    Write-Host "`n[4/11] Downloading Google Test v$GTEST_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[4/11] Google Test already present" -ForegroundColor Green
}
...
if (-Not (Test-Path $DBMATE_EXE)) {
    Write-Host "`n[5/11] Downloading dbmate v$DBMATE_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[5/11] dbmate already present" -ForegroundColor Green
}
...
if (-Not (Test-Path $FLATC_EXE)) {
    Write-Host "`n[6/11] Downloading flatc v$FLATBUFFERS_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[6/11] flatc already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $FLATBUFFERS_DIR "include\flatbuffers\flatbuffers.h"))) {
    Write-Host "`n[7/11] Downloading FlatBuffers sources v$FLATBUFFERS_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[7/11] FlatBuffers headers already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $IXWS_DIR "ixwebsocket\IXWebSocket.h"))) {
    Write-Host "`n[8/11] Downloading ixwebsocket v$IXWS_VERSION..." -ForegroundColor Yellow
    ...
    Write-Host "`n[8/11] ixwebsocket already present" -ForegroundColor Green
}
...
if (-Not (Test-Path (Join-Path $LIBSODIUM_DIR "include\sodium.h"))) {
    Write-Host "`n[9/11] Downloading libsodium v$LIBSODIUM_VERSION (msvc prebuilt)..." -ForegroundColor Yellow
    ...
    Write-Host "`n[9/11] libsodium already present" -ForegroundColor Green
}

# ────────────────────────────────────────────────────────────
# 10. moodycamel::concurrentqueue (header-only MPMC queue — step-006 sim cmd queue)
# ────────────────────────────────────────────────────────────
$CONCURRENTQUEUE_VERSION = "1.0.4"
$CONCURRENTQUEUE_DIR = Join-Path $VENDOR "concurrentqueue"

if (-Not (Test-Path (Join-Path $CONCURRENTQUEUE_DIR "concurrentqueue.h"))) {
    Write-Host "`n[10/11] Downloading concurrentqueue v$CONCURRENTQUEUE_VERSION..." -ForegroundColor Yellow
    $cqUrl = "https://github.com/cameron314/concurrentqueue/archive/refs/tags/v$CONCURRENTQUEUE_VERSION.zip"
    $cqZip = Join-Path $VENDOR "concurrentqueue.zip"

    Invoke-WebRequest -Uri $cqUrl -OutFile $cqZip -UseBasicParsing
    Expand-Archive -Path $cqZip -DestinationPath $VENDOR -Force

    if (Test-Path $CONCURRENTQUEUE_DIR) { Remove-Item -Recurse -Force $CONCURRENTQUEUE_DIR }
    Rename-Item -Path (Join-Path $VENDOR "concurrentqueue-$CONCURRENTQUEUE_VERSION") -NewName "concurrentqueue"
    Remove-Item $cqZip

    Write-Host "  concurrentqueue ready" -ForegroundColor Green
} else {
    Write-Host "`n[10/11] concurrentqueue already present" -ForegroundColor Green
}

# ────────────────────────────────────────────────────────────
# 11. PostgreSQL install detection (for libpq headers + import lib)
# ────────────────────────────────────────────────────────────
Write-Host "`n[11/11] Detecting local PostgreSQL install..." -ForegroundColor Yellow
$PG_ROOT = $env:PGROOT
if (-Not $PG_ROOT) {
